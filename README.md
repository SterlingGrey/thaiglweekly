...
2. Official-channel YouTube thumbnails for a trailer already on the page, hotlinked from `https://img.youtube.com/vi/{id}/hqdefault.jpg` (`maxresdefault` is missing for many uploads). The file stays on Google's servers and is tied to the embed already on the card. Today that is 70 of 78 series; the rest are listed in `data/missing-trailers.txt`. The slot is 16:9 in CSS. Every still lazy-loads, Tonight's included, to keep the first load under 100 KB.
